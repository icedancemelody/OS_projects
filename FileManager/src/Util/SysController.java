...
public class SysController {
    ...
    public void start() {
        ...
        printBlank("不知道怎么使用？请输入 help 查看命令帮助信息", null);
    ...
    public void printBlank(String command, String instruction) {
        //打印空格
        //command   命令
        //instruction   说明
    ...
    //前进更改目录
    private void processChangeInDir(String[] param) {
        if (param.length > 2) {
            printBlank("命令使用错误，输入-HELP查看命令使用方法", null);
        } else if (param.length > 1) {
            if (param[1].equals("-help")) {
        ...
        if (param.length > 2) {
            ...
            if (param[1].equals("-help")) {
                ...
                if (_currentPath.searchFCB_ListByName(param[1]) == null) {
                    fcbController.createFolder(param[1], currentPath + "/" + param[1], SysController._currentPath);
    ...
    //格式化
    private void processFormat(String[] param) {
        if (param.length > 2) {
            printBlank("命令使用错误，输入-HELP查看命令使用方法", null);
        } else if (param.length > 1) {
            if (param[1].equals("-help")) {
            ...
            //先让路径回到根目录：
            while (SysController._currentPath != rootFolderMark) {
                SysController._currentPath = _currentPath.getFatherFolder();
                SysController.currentPath = _currentPath.getFolderPath();
            }
            Iterator iterator = _currentPath.getFcb_list().iterator();
            SysFile tempFile;
            FCB_List tempFolder;
            List<String> sbDir = new ArrayList<String>();
            List<String> sbFile = new ArrayList<String>();
            while (iterator.hasNext()) {
                Object obj = iterator.next();
                try {
                    tempFile = (SysFile) obj;
                    ...
                    sbFile.add(tempFileName);
                } catch (Exception e) {
                    tempFolder = (FCB_List) obj;
                    int folderLength = tempFolder.getFcb_list_name().getBytes().length;
                    String tempFileFolderName = tempFolder.getFcb_list_name();
                    sbDir.add(tempFileFolderName);
                }

            }
            //删除根目录下所有文件：
            int dirSize = sbDir.size();
            int fileSize = sbFile.size();
            for (int i = 0; i < dirSize; i++) {
                String tempDir = sbDir.remove(0);
                String[] tempToDelete = new String[]{"deldir", tempDir};
                processDeleteDir(tempToDelete);
            }
            for (int i = 0; i < fileSize; i++) {
                String tempfile = sbFile.remove(0);
                String[] tempToDelete = new String[]{"deldir", tempfile};
                processDeleteFile(tempToDelete);
            }
    ...
    public void helpInformation() {
        ...
        printBlank("MFN fileName", "修改名字为fileName的文本文件的名称");
        printBlank("DEL fileName", "删除名字为fileName的文本文件");
        printBlank("CDIR folderName", "创建名字为folderName(不超过25个字符)的文件夹（子目录）");
        printBlank("CD folderName", "进入名字为folderName的文件夹（更改目录）,..为父目录");
        printBlank("MDN folderName", "修改名字为folderName的文件夹的名称");
        printBlank("DELDIR folderName", "删除名字为folderName的文件夹（子目录）");
        printBlank("LS", "显示当前文件夹下的所有文件");
        printBlank("FM", "格式化（删除根目录下的所有文件）");
        System.out.println("-------------------------------------------------------------------------");
    }
